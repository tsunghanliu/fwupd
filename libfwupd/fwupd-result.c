 ...
 * the Free Software Foundation; either device_version 2 of the License, or
 * (at your option) any later device_version.
 *
 ...
 * @result: A #FwupdResult
 * @update_trust_flags: the trust flags, e.g. %FWUPD_TRUST_FLAG_PAYLOAD
 *
 ...
 **/
void
fwupd_result_set_update_trust_flags (FwupdResult *result, FwupdTrustFlags update_trust_flags)
{
	FwupdResultPrivate *priv = GET_PRIVATE (result);
	g_return_if_fail (FWUPD_IS_RESULT (result));
	priv->update_trust_flags = update_trust_flags;
}
