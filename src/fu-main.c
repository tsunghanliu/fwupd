...
fu_main_get_plugin_for_device (GHashTable *plugins, FuDevice *device)
{
	...
	tmp = fu_device_get_metadata (device, FU_DEVICE_KEY_VENDOR_DRIVER);
	if (tmp == NULL)
		return NULL;
				  ...
				  gpointer user_data)
{
	...
	if (app != NULL) {
		const gchar *tmp;
		tmp = as_app_get_metadata_item (app, FU_DEVICE_KEY_VENDOR_DRIVER);
		if (tmp != NULL) {
			g_debug ("setting vendor plugin: %s", tmp);
			fu_device_set_metadata (item->device,
						FU_DEVICE_KEY_VENDOR_DRIVER,
						tmp);
		}
	}
