...
dfu_target_set_usb_dev_iface (DfuTarget *target, GUsbDevice *dev, GUsbInterface *iface)
{
	...
	if (g_bytes_get_size (iface_data) != 0x09) {
		g_warning ("interface data invalid size: %li",
			   g_bytes_get_size (iface_data));
		return FALSE;
	}
	...
	desc = g_bytes_get_data (iface_data, NULL);
...
dfu_target_close (DfuTarget *target, GError **error)
{
	...
	/* release */
	if (!g_usb_device_release_interface (priv->dev, (gint) priv->iface_number, 0, &error_local)) {
		g_set_error (error,
			     FWUPD_ERROR,
			     FWUPD_ERROR_INTERNAL,
			   ...
			   GCancellable *cancellable, GError **error)
{
	...
	for (i = 0; i < max_ms / poll_interval_ms; i++) {
		...
		if (vid != g_usb_device_get_vid (dev_tmp) ||
			...
			iface = g_usb_device_get_interface (dev_tmp,
 ...
 **/
GBytes *
dfu_target_upload (DfuTarget *target, GCancellable *cancellable, GError **error)
{
	DfuTargetPrivate *priv = GET_PRIVATE (target);
	GBytes *chunk_tmp;
	gsize chunk_size;
 ...
 * dfu_target_upload:
 * @target: a #DfuTarget
 * @flags: flags to use, e.g. %DFU_TARGET_DOWNLOAD_FLAG_VERIFY
 * @cancellable: a #GCancellable, or %NULL
 * @error: a #GError, or %NULL
 *
		     ...
		     GCancellable *cancellable, GError **error)
{
	...
	/* verify */
	if (flags & DFU_TARGET_DOWNLOAD_FLAG_VERIFY) {
		g_autoptr(GBytes) bytes_tmp = NULL;
		bytes_tmp = dfu_target_upload (target, cancellable, error);
		if (bytes_tmp == NULL)
			return FALSE;
	...
	/* do a host reset */
	if (flags & DFU_TARGET_DOWNLOAD_FLAG_HOST_RESET) {
		if (!dfu_target_reset (target, error))
			return FALSE;
	}
