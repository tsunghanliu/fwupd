...
typedef enum {
	DFU_SECTOR_CAPABILITY_NONE		= 0,
	DFU_SECTOR_CAPABILITY_READABLE		= 1 << 0,
	DFU_SECTOR_CAPABILITY_WRITEABLE		= 1 << 1,
	DFU_SECTOR_CAPABILITY_ERASEABLE		= 1 << 2,
	DFU_SECTOR_CAPABILITY_LAST
} DfuSectorCapability;
...
dfu_target_init (DfuTarget *target)
{
	...
	priv->sectors = g_ptr_array_new_with_free_func (g_free);
}
...
/**
 * dfu_target_sectors_to_string: (skip)
 **/
gchar *
dfu_target_sectors_to_string (DfuTarget *target)
{
	...
	for (i = 0; i < priv->sectors->len; i++) {
		sect = g_ptr_array_index (priv->sectors, i);
		g_string_append_printf (str,
					"Zone:%i, Sec#:%i, Addr:0x%08x, "
					"Size:0x%04x, Caps:0x%01x\n",
					sect->zone, sect->number, sect->addr,
					sect->size, sect->cap);
	}
	if (str->len > 0)
...
{
	DfuTargetPrivate *priv = GET_PRIVATE (target);
	DfuSectorCapability cap = DFU_SECTOR_CAPABILITY_NONE;
	gchar *tmp;
	...
	switch (tmp[1]) {
	case 'a':
		cap = DFU_SECTOR_CAPABILITY_READABLE;
		break;
	case 'b':
		cap = DFU_SECTOR_CAPABILITY_ERASEABLE;
		break;
	case 'c':
		cap = DFU_SECTOR_CAPABILITY_READABLE |
		      DFU_SECTOR_CAPABILITY_ERASEABLE;
		break;
	case 'd':
		cap = DFU_SECTOR_CAPABILITY_WRITEABLE;
		break;
	case 'e':
		cap = DFU_SECTOR_CAPABILITY_READABLE |
		      DFU_SECTOR_CAPABILITY_WRITEABLE;
		break;
	case 'f':
		cap = DFU_SECTOR_CAPABILITY_ERASEABLE |
		      DFU_SECTOR_CAPABILITY_WRITEABLE;
	...
	for (i = 0; i < nr_sectors; i++) {
		DfuSector *sect;
		sect = g_new0 (DfuSector, 1);
		sect->addr = addr + addr_offset;
		sect->size = sector_size;
		sect->size_left = (nr_sectors * sector_size)- addr_offset;
		sect->zone = zone;
		sect->number = sector;
		sect->cap = cap;
		g_ptr_array_add (priv->sectors, sect);
		 ...
		 GCancellable *cancellable, GError **error)
{
	...
	if (priv->sectors->len == 0) {
		DfuSector *sect = g_new0 (DfuSector, 1);
			   ...
			   GError **error)
{
	...
	gsize total_size = 0;
	guint8 *buffer;
	guint dfuse_last_sector = G_MAXUINT;
	...
	for (i = 0; i < 0xffff; i++) {
		...
		if (priv->dfuse_supported) {
			...
			if ((sect->cap & DFU_SECTOR_CAPABILITY_READABLE) == 0) {
				g_set_error (error,
					     DFU_ERROR,
					     DFU_ERROR_INVALID_DEVICE,
		   ...
		   GError **error)
{
	...
	DfuSector *sect;
	guint i;
	guint last_sector_number = G_MAXUINT;
	guint last_sector_zone = G_MAXUINT;
	...
	for (i = 0; i < priv->sectors->len; i++) {
		...
		sect = g_ptr_array_index (priv->sectors, i);
		if (sect->zone == last_sector_zone)
			continue;
		if (sect->number == last_sector_number)
			continue;

		/* get the first element from the hardware */
		g_debug ("starting upload from 0x%08x (0x%04x)",
			 sect->addr, sect->size_left);
		...
		last_sector_zone = sect->zone;
		last_sector_number = sect->number;
	}
			     ...
			     GError **error)
{
	...
	for (i = 0; i < nr_chunks + 1; i++) {
		...
		if (priv->dfuse_supported) {
			...
			if ((sect->cap & DFU_SECTOR_CAPABILITY_WRITEABLE) == 0) {
				g_set_error (error,
					     DFU_ERROR,
					     DFU_ERROR_INVALID_DEVICE,
